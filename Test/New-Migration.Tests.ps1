...
BeforeAll {
    ...
    $script:rivetPath = Join-Path $PSScriptRoot -ChildPath '..\Rivet\rivet.ps1' -Resolve
}
...
Describe 'New-Migration' {
    ...
    It 'should create one migration' {
        & $script:rivetPath -New -Name 'ShouldCreateOneMigration' -Database $RTDatabaseName -ConfigFilePath $RTConfigFilePath
        $? | Should -BeTrue
    ...
    It 'should create multiple migrations' {
        ...
        & $script:rivetPath -New -Name 'ShouldCreateMultipleMigrations' -Database $RTDatabaseName,RivetTestTwo -ConfigFilePath $RTConfigFilePath
        $? | Should -BeTrue
        $LASTEXITCODE | Should -Be 0
    ...
    It 'should create migration in all databases' {
        ...
        & $script:rivetPath -New -Name 'ShouldCreateMigrationAcrossAllDatabases' -ConfigFilePath $RTConfigFilePath
    ...
    It 'should require database name if new database' {
        ...
        & $script:rivetPath -New -Name 'ShouldCreateMigrationAcrossAllDatabases' -ConfigFilePath $RTConfigFilePath -ErrorAction SilentlyContinue
    ...
    It 'should create database directory if it does not exist' {
        & $script:rivetPath -New -Name 'ShouldCreateMigrationForNewDatabase' -Database 'NewDatabase' -ConfigFilePath $RTConfigFilePath
        $LASTEXITCODE | Should -Be 0
    ...
    It 'should create migrations with unique ids' {
        $m1 = & $script:rivetPath -New -Name 'First' -ConfigFilePath $RTConfigFilePath
        $m2 = & $script:rivetPath -New -Name 'Second' -ConfigFilePath $RTConfigFilePath
    ...
    It 'should reject migrations with names that are too long' {
        ...
        try
        {
            & $script:rivetPath -New -Name $name -ConfigFilePath $RTConfigFilePath
    ...
    It 'should handle new migration for ignored database' {
        & $script:rivetPath -New -Name 'Migration' -Database 'Ignored' -ConfigFilePath $RTConfigFilePath -ErrorAction SilentlyContinue
        $Global:Error.Count | Should -BeGreaterThan 0
    }

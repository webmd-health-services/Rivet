...
function Set-StoredProcedure
{
    ...
    .DESCRIPTION
    The stored procedure should be in the `$Database\Stored Procedures` directory, in a file with the same name as the stored procedure, e.g. `$Name.sql`.  If the schema is *not* `dbo`, it should be at the front of the filename, e.g. `pstep.Migrators`.
    ...
    .EXAMPLE
    Set-StoredProcedure -Name InsertMigration -Schema pstep
    
    Drops the `pstep.InsertMigration` stored procedure (it it exists), then executes the contents of the `$Database\Stored Procedures\pstep.InsertMigration.sql` file.
    #>
    [CmdletBinding()]
    param(
